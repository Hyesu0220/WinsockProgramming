...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		...
		// 데이터 크기 보내기
		iResult = send(connectSocket, (char*)&length, sizeof(int), 0);
		if (iResult == SOCKET_ERROR)
		{
		...
		printf("[TCP 클라이언트] 데이터 크기를 보냈습니다. (%d Bytes)\n", length);

		// 데이터 보내기
		iResult = send(connectSocket, buf, length, 0);
		if (iResult == SOCKET_ERROR)
		{
			err_display("send failed.");
			break;
		}
		printf("[TCP 클라이언트] 데이터 %d Bytes를 보냈습니다.\n", iResult);
		memset(&buf, 0, sizeof(buf));

		iResult = RecvFixedSize(connectSocket, buf, length, 0);
		if (iResult == SOCKET_ERROR)
		{
